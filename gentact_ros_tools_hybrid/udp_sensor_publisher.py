...
class UDPSensorPublisher(Node):
    def __init__(self):
        ...
        self.max_devices = (
            self.get_parameter("max_devices").get_parameter_value().integer_value
        )
        ...
        self.socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 4 * 1024 * 1024)
        ...
        if device_id not in self.device_publishers:
            if len(self.device_publishers) >= self.max_devices:
                self.get_logger().warn(
                    f"Maximum devices ({self.max_devices}) reached, ignoring device {device_id}"
                )
                return None
            ...
            self.get_logger().info(
                f"New device connected: {device_id:08x} -> {topic_name}"
            )
