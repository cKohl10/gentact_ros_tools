    ...
    if config["controller"]["active"]:
        ...
        controller_nodes.append(
            Node(
                ...
                output="screen",
                parameters=[
                    {"avoidance_type": avoidance_type, "movement_type": movement_type}
                ],
            )
        )
...
def build_robot_description(config):
    ...
    if isinstance(config["robot"]["end_effector"], dict) and config["robot"][
        "end_effector"
    ].get("active", False):
    ...
    urdf_file = PathJoinSubstitution(
        [FindPackageShare("gentact_ros_tools_hybrid"), config["robot"]["robot_xacro"]]
    )
    ...
    for sensor_key, sensor_config in config["sensors"].items():
        if isinstance(sensor_config, dict) and sensor_config.get("active", False):
            if sensor_config.get("type", "") == "SPAD":
                ...
                serial_port = sensor_port_mapping.get(
                    sensor_key, sensor_config.get("port", "")
                )
...
def build_joint_relay_nodes(config):
    ...
    if config["robot"]["franky_joint_relay"]:
        ...
        joint_relay_nodes.append(
            Node(  # Relays /joint_states to the robot's namespace
                package="gentact_ros_tools_hybrid",
...
def launch_setup(context, *args, **kwargs):
    ...
    for sensor_key, sensor_config in config["sensors"].items():
        if sensor_key in sensor_port_mapping:
            ...
            if (
                ...
                and sensor_config.get("active", False)
                and sensor_config.get("type", "") == "SCPS"
            ):
                sensor_prediction_nodes = build_prediction_nodes(
                    config, sensor_key, sensor_config
                )
    ...
    for sensor_key, sensor_config in config["sensors"].items():
        if sensor_key in sensor_port_mapping:
            ...
            if (
                ...
                sensor_prediction_nodes = build_tracker_nodes(
                    config, sensor_key, sensor_config
                )
    ...
    for joint_relay_node in joint_relay_nodes:
        launch_actions.append(
            TimerAction(period=timer_period, actions=[joint_relay_node])
        )
        timer_period += timer_period_delay
...
def generate_launch_description():
    ...
    return LaunchDescription(
        [config_file_arg, use_sim_time_arg, OpaqueFunction(function=launch_setup)]
    )
                ...
                while time.time() - start_time < 2.0:
                    if ser.in_waiting > 0:
                        ...
                        if line:
                            ...
                            if (
                                first_element
                                and "S" in first_element
                                and first_element.replace("S", "").isdigit()
                            ):
    ...
    for sensor_key, sensor_config in config["sensors"].items():
        if isinstance(sensor_config, dict) and sensor_config.get("active", False):
            ...
            if expected_serial and expected_serial in port_mapping:
                ...
                print(
                    f"Mapped {sensor_key} (SN: {expected_serial}) to {port_mapping[expected_serial]}"
                )
            else:
                print(
                    f"Warning: No port found for {sensor_key} with SN: {expected_serial}"
                )
                ...
                if "port" in sensor_config:
                    if sensor_config["port"] == "":
                        ...
                        print(
                            f"Using fallback port {sensor_config['port']} for {sensor_key}"
                        )
